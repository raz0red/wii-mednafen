...
static void select_language() {
    ...
    if (lang->node_type == NODETYPE_LANG_DEFAULT) {
        ...
        snprintf(langfile, WII_MAX_PATH, "%s%s.%s", wii_get_lang_dir(),
                 lang->name, WII_LANG_EXT);
...
void wii_mednafen_menu_init() {
    ...
    TREENODE* emulator_settings =
        wii_create_tree_node(NODETYPE_EMULATOR_SETTINGS, "Emulator settings");
    ...
    TREENODE* video_settings =
        wii_create_tree_node(NODETYPE_VIDEO_SETTINGS, "Video settings");
...
void wii_menu_handle_get_node_name(TREENODE* node, char* buffer, char* value) {
    ...
    switch (node->node_type) {
        ...
        case NODETYPE_VOLUME:
            snprintf(value, WII_MENU_BUFF_SIZE, "%d %s", (wii_volume / 10),
...
void wii_menu_handle_select_node(TREENODE* node) {
    ...
    if (node->node_type == NODETYPE_ROM || node->node_type == NODETYPE_RESUME ||
 ...
 *
 * @param   menu The menu to read the languages into
 */
static void read_lang_list(TREENODE* menu) {
...
static void wii_read_game_list(TREENODE* menu) {
    ...
    if (strlen(roms) > 0) {
        ...
        if (romdir != NULL) {
            ...
            char msg[256];
            snprintf(msg, sizeof(msg), "%s: %s", gettextmsg("Error opening"),
                     roms);
            wii_set_status_message(msg);
        }
    }
